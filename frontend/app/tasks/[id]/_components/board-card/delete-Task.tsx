...
export const DeleteTask: React.FC<DeleteTaskProps> = ({id}) => {
  ...
  async function submitHandler(id: string) {
    ...
    try{
      await deleteTask(id);
      router.refresh();
      toast(`Task Deleted!`, { description: <span>{getCurrentDate()}</span> })
    } catch (error) {
      console.log(error);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        ...
        {/* <span className="flex w-20 mr-2 border-2 rounded-md hover:bg-red-400 p-1 place-items-center justify-center cursor-pointer"> <TrashIcon /> delete </span> */}

        <TrashIcon width={30} height={30} className="border-2 rounded-md hover:bg-red-900 text-white p-1 cursor-pointer" />
      </DialogTrigger>
